...
@Service
public class NeteaseStockClient {
...
//	@Override
	public Stock getStockById(String stockId) throws Exception {
		...
		if(stocks != null && stocks.size() >= 1) {
			stock = stocks.get(0);
			stock.setDailyKLineAddr(getKLineAddrById(convertStockId(stock.getId()), KLineType.DAY));
			//reverse stock Id
...
//	@Override
	public List<Stock> getStockBatchByIds(String stockIds) throws Exception {
		...
		for(Stock stock : stocks) {
			stock.setDailyKLineAddr(getKLineAddrById(convertStockId(stockIds), KLineType.DAY));
			//reverse stock Id
			stock.setId(restoreConvertStockId(stock.getId()));
		}
		return stocks;
